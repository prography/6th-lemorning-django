...
def build_musicnn(x, is_training, num_classes, num_filt_frontend=1.6, num_filt_midend=64, num_units_backend=200):
    ...
    frontend_features_list = frontend(
        x, is_training, config.N_MELS, num_filt=1.6, type='7774timbraltemporal')
    # concatnate features coming from the front-end
    frontend_features = tf.concat(frontend_features_list, 2)

    # mid-end ### dense layers
    midend_features_list = midend(
        frontend_features, is_training, num_filt_midend)
    ...
    logits, penultimate, mean_pool, max_pool = backend(
        midend_features, is_training, num_classes, num_units_backend, type='globalpool_dense')

    # [extract features] temporal and timbral features from the front-end
    timbral = tf.concat(
        [frontend_features_list[0], frontend_features_list[1]], 2)
    temporal = tf.concat([frontend_features_list[2],
                          frontend_features_list[3], frontend_features_list[4]], 2)
...
def frontend(x, is_training, yInput, num_filt, type):
    ...
    if 'timbral' in type:
        ...
        input_pad_7 = tf.pad(normalized_input, [
                             [0, 0], [3, 3], [0, 0], [0, 0]], "CONSTANT")
...
def timbral_block(inputs, filters, kernel_size, is_training, padding="valid", activation=tf.nn.relu):
    ...
    bn_conv = tf.compat.v1.layers.batch_normalization(
        conv, training=is_training)
...
def midend(front_end_output, is_training, num_filt):
    ...
    bn_conv2_pad = tf.pad(
        res_conv2, [[0, 0], [3, 3], [0, 0], [0, 0]], "CONSTANT")
    conv3 = tf.compat.v1.layers.conv2d(inputs=bn_conv2_pad,
                                       filters=num_filt,
                                       kernel_size=[7, bn_conv2_pad.shape[2]],
                                       padding="valid",
                                       activation=tf.nn.relu)
    bn_conv3 = tf.compat.v1.layers.batch_normalization(
        conv3, training=is_training)
...
def backend(feature_map, is_training, num_classes, output_units, type):
    ...
    flat_pool = tf.compat.v1.layers.batch_normalization(
        flat_pool, training=is_training)
    ...
    dense = tf.compat.v1.layers.dense(inputs=flat_pool_dropout,
                                      units=output_units,
                                      activation=tf.nn.relu)
    bn_dense = tf.compat.v1.layers.batch_normalization(
...
def vgg(x, is_training, num_classes, num_filters=32):
    ...
    do_pool1 = tf.compat.v1.layers.dropout(
        pool1, rate=0.25, training=is_training)
    ...
    do_pool2 = tf.compat.v1.layers.dropout(
        pool2, rate=0.25, training=is_training)
    ...
    do_pool3 = tf.compat.v1.layers.dropout(
        pool3, rate=0.25, training=is_training)
    conv4 = tf.compat.v1.layers.conv2d(inputs=do_pool3,
                                       ...
                                       activation=tf.nn.relu,
                                       name='4CNN')
    bn_conv4 = tf.compat.v1.layers.batch_normalization(
        conv4, training=is_training)
    pool4 = tf.compat.v1.layers.max_pooling2d(
        inputs=bn_conv4, pool_size=[2, 2], strides=[2, 2])

    do_pool4 = tf.compat.v1.layers.dropout(
        pool4, rate=0.25, training=is_training)
    conv5 = tf.compat.v1.layers.conv2d(inputs=do_pool4,
                                       ...
                                       activation=tf.nn.relu,
                                       name='5CNN')
    bn_conv5 = tf.compat.v1.layers.batch_normalization(
        conv5, training=is_training)
    ...
    do_pool5 = tf.compat.v1.layers.dropout(
        flat_pool5, rate=0.5, training=is_training)
